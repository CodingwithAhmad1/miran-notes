...
**Count-based cap with graceful pruning:** `UndoPolicy.maxUndoSteps` (default **200**) caps the number of undo steps. `AppModel` keeps a **checkpoint timeline** (`undoCheckpoints`): one full `NoteDocument` per document version on the retained branch, so consecutive undo boundaries are **not** duplicated as separate before/after pairs. Action names for the menu mirror the previous `undoHistory` list (`[String]`). When the step count exceeds the cap, the **oldest** checkpoints are dropped, `NSUndoManager.removeAllActions(withTarget:)` runs, and surviving steps are re-registered in order. `clearUndoStack` resets both the manager and checkpoints.
...
**Memory estimate (diagnostics / tests):** `NoteDocument.estimatedUndoMemoryBytes` approximates retained size from UTF-16 length and metadata counts. `NoteDocument.estimatedUndoBytes(forCheckpoints:)` sums checkpoints. Worst-case retained snapshots are **at most `maxUndoSteps + 1` full documents** on the undo branch (fewer when coalescing reduces steps). Automated checks: [`AppModelUndoMemoryTests`](Tests/MiranNotesAppTests/AppModelUndoMemoryTests.swift), [`NoteDocumentUndoMemoryTests`](Tests/MiranNotesTests/NoteDocumentUndoMemoryTests.swift).

**Highlighted limitations (must stay explicit in product and code):**

1. **Memory** — Undo still stores **full `NoteDocument` checkpoints** (not diffs). The step cap and checkpoint sharing bound worst-case growth to **O(maxUndoSteps × document size)** in the estimate above; the 1 MB UTF-16 note cap bounds per-snapshot text size.
